...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let insertSupporter = (supporterName, supporterEmail) => {
        return new Promise((resolve, reject) => {
            pool.query("INSERT INTO Supporter (supporterName, supporterEmail, budget) VALUES(?,?,?)", [supporterName, supporterEmail, 0], (error, rows) => {
    ...
    try {
        ...
        if (exists) {
            ...
            response.error = "Couldn't register " + info.name;
        } else {
            // INSERT
            const inserted = await insertSupporter(info.name, info.email);
            if (inserted) {
