...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let insertDesigner = (designerName, designerEmail) => {
        return new Promise((resolve, reject) => {
            pool.query("INSERT INTO Designer (designerName,designerEmail) VALUES(?,?)", [designerName, designerEmail], (error, rows) => {
    ...
    try {
        ...
        const exists = await CheckDesignerExistence(info.name, info.email);
        ...
        if (exists) {
            ...
            // INSERT
            const inserted = await insertDesigner(info.name, info.email);
            if (inserted) {
