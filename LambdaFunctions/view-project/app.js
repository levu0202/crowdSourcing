...
exports.lambdaHandler = async (event, context, callback) => {
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let getSupporterActivityForProject = (projectName) => {
            return new Promise((resolve, reject) => {
                pool.query("SELECT * FROM ClaimPledgeActivity WHERE projectName=?", [projectName], (error, rows) => {
                    if (error) { return reject(error); }
                    return resolve(rows);
                });
            });
    ...
    try {
        ...
        const spActivity = await getSupporterActivityForProject(info.projectName);
        ...
        if (false) {
            ...
            resultDict["supportActivity"] = spActivity;
